...
from lanenet.train import TrainLaneNet
...
class Runner(object):
    def __init__(self, cfg):
        ...
        if self.cfg.train_lanenet:
            self.lanenet = TrainLaneNet(self.cfg)
    ...
    def resume(self):
        if not self.cfg.load_from and not self.cfg.finetune_from:
            return
        load_network(self.net, self.cfg.load_from, finetune_from=self.cfg.finetune_from, lanenet_ckp=self.cfg.lanenet_ckp, logger=self.recorder.logger)

    def train_epoch(self, epoch, train_loader):
        self.net.train()
        if self.cfg.train_lanenet: self.lanenet.model.train() 
        end = time.time()
        ...
        for i, data in enumerate(train_loader):
            ...
            output, output_predictions = self.net(data)
            ...
            if self.cfg.train_lanenet: # lanenet train
                lanenet_loss = self.lanenet.train_batch(data, output, output_predictions)
                output['loss_stats'].update({'lanenet_loss':lanenet_loss})
        ...
        if self.cfg.train_lanenet:
            torch.save(self.lanenet.model.state_dict(), f'lanenet/ckp/{epoch}.pth')
    ...
    def demo(self):
        if not self.test_loader:
            self.test_loader = build_dataloader(self.cfg.dataset.demo,
                                                self.cfg,
                                                is_train=False)
        self.net.eval()
        predictions = []
        for i, data in enumerate(tqdm(self.test_loader, desc=f'Demo')):
            data = self.to_cuda(data)
            with torch.no_grad():
                output = self.net(data)
                output = self.net.module.heads.get_lanes(output)
