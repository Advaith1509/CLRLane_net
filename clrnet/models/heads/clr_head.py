...
from lanenet.inference import LaneNetAssign
from lanenet.generate_data import GeneratePairs
...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        if not self.cfg.train_lanenet: 
            self.lanenet_assign = LaneNetAssign(self.cfg)
        else:
            self.lanenet_batch_size = self.cfg.lanenet_batch_size
            self.generate_data = GeneratePairs(self.cfg)
    ...
    def forward(self, x, **kwargs):
        ...
        if self.training:
            ...
            return self.loss(output, kwargs['batch']), output['predictions_lists']
    ...
    def loss(self,
        ...
        sel_pred_ind_batch = []
        cls_acc_stage = []
        if self.cfg.train_lanenet:
            sel_gt_ind_batch = torch.zeros((self.lanenet_batch_size,1,1)).cuda()
            pair_features_batch = torch.zeros((self.lanenet_batch_size,1,6)).cuda()
            pair_match_batch = torch.zeros((self.lanenet_batch_size,1,1)).cuda()
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                # label assignment                
                with torch.no_grad():
                    if self.cfg.train_lanenet: # classic cost function assign
                        matched_row_inds, matched_col_inds, cost_input, cost_target, cost_gt, cost_lane_idx = assign(
                                     predictions, target, self.img_w, self.img_h, train_lanenet = True, lanenet_batch=self.lanenet_batch_size, generate_data=self.generate_data)
                        sel_gt_ind_batch = torch.cat((sel_gt_ind_batch,cost_target[:,None,:]),dim=1)
                        pair_features_batch = torch.cat((pair_features_batch,cost_input[:,None,:]),dim=1)
                        pair_match_batch = torch.cat((pair_match_batch,cost_gt[:,None,:]),dim=1)
                        sel_pred_ind_batch.extend(cost_lane_idx)
                    
                    else: # lanenet assign
                        self.lanenet_assign.model.eval()
                        matched_row_inds, matched_col_inds = assign(predictions, target, self.img_w, self.img_h, train_lanenet = False, 
                                                                    lanenet_assign = self.lanenet_assign)                        
                ...
                # Loss calculation
                if reg_yxtl.numel()!=0: # only not empty assignments
                    ...
                    iou_loss = iou_loss + liou_loss(
                        ...
                        self.img_w, length=15).mean()
                else:
                    cls_loss = cls_loss
                    reg_xytl_loss = reg_xytl_loss
                    iou_loss = iou_loss
        ...
        if self.cfg.train_lanenet:
            return_value.update({'pair_features_batch': pair_features_batch,
                                           'sel_gt_ind_batch': sel_gt_ind_batch,
                                           'pair_match_batch': pair_match_batch,
                                           'sel_pred_ind_batch': sel_pred_ind_batch})
