...
def focal_cost(cls_pred, gt_labels, alpha=0.25, gamma=2, eps=1e-12):
    ...
    return cls_cost, pos_cost[:, gt_labels], neg_cost[:, gt_labels]
...
def assign(
    ...
    distances_score_deep = distances_score.clone().detach()
    distances_score = 1 - (distances_score / torch.max(distances_score)
                           ) + 1e-2  # normalize the distance

    # classification cost
    cls_score, pos_score, neg_score = focal_cost(predictions[:, :2], targets[:, 1].long())
    pos_score_deep = pos_score.clone().detach()
    neg_score_deep = neg_score.clone().detach()
    num_priors = predictions.shape[0]
    num_targets = targets.shape[0]
    ...
    start_xys_score_deep = start_xys_score.clone().detach()
    ...
    theta_score_deep = theta_score.clone().detach()
    ...
    iou_deep = iou.clone().detach()

    if train_lanenet: 
        # classic cost function assign
        matched_row_inds, matched_col_inds = dynamic_k_assign(cost, iou)
        # generate data for lanenet training
        cost_input, lane_target, gt, lane_idx  = generate_data(
                                                theta_score_deep, start_xys_score_deep, distances_score_deep,
                                                pos_score_deep, neg_score_deep, iou_deep, matched_row_inds, 
                                                matched_col_inds, lanenet_batch)
        return matched_row_inds, matched_col_inds, cost_input, lane_target, gt, lane_idx 

    else:
        # lanenet assign
        matched_row_inds, matched_col_inds = lanenet_assign(
                                            theta_score_deep, start_xys_score_deep, distances_score_deep,
                                            pos_score_deep, neg_score_deep, iou_deep, targets)
