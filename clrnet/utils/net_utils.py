...
def load_network_specified(net, model_dir, lanenet_dir=None, logger=None):
    # CLRNet 
    ...
    # LaneNet
    if lanenet_dir is not None:
        logger.info('Using lanenet as the label assignment method. Loading lanenet checkpoint from: ' + lanenet_dir)
        pretrained_lanenet = torch.load(lanenet_dir)
        for k, v in pretrained_lanenet.items():
            adapted_k = 'module.heads.lanenet_assign.model.'+k
            if adapted_k not in net_state.keys() or v.size() != net_state[adapted_k].size():
                continue
            state[adapted_k] = v
    else:
        logger.info('Training lanenet')

    net.load_state_dict(state, strict=False)


def load_network(net, model_dir, finetune_from=None, lanenet_ckp = None, logger=None):
    if finetune_from:
        if logger:
            logger.info('Finetune model from: ' + finetune_from)
        load_network_specified(net, finetune_from, lanenet_ckp, logger)
        return
